"""ApiKey.budget_limit_cents must be enforced, not dead code.

Before this fix the column was loaded into KeyContext and never consulted:
a leaked key meant unbounded spend on the operator's upstream accounts.
The cap is checked pre-flight in execute_chat against recorded spend in
requests_log — request-granular, so the overshoot is bounded by one
request's own cost.
"""
...
async def test_exhausted_budget_returns_429_before_upstream(tmp_sqlite_url, monkeypatch):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://t") as client:
        ...
        # No recorded spend → the key gets past the budget gate. There are
        # no providers configured in this fixture, so the engine rejects
        # downstream with 422 — anything but 429 proves the gate passed.
        resp = await client.post(
            "/v1/chat/completions",
            json={"model": "gpt-4o-mini", "messages": [{"role": "user", "content": "hi"}]},
...
async def test_key_without_budget_is_never_blocked(tmp_sqlite_url, monkeypatch):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://t") as client:
        ...
        # NULL budget → no spend cap at all. The request fails downstream
        # (no providers configured in this fixture) but never with 429.
        assert resp.status_code != 429
