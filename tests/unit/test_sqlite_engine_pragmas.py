"""The default SQLite engine must survive concurrent write traffic.

Issue: stock SQLite (DELETE journal, zero busy timeout) turns any two
overlapping write transactions — AuthMiddleware's last_used_at commit
racing a RequestLog insert — into "database is locked" → 500/503. The
engine now enables WAL + synchronous=NORMAL + a 30s busy timeout on
every connection.
"""
...
async def test_concurrent_writers_do_not_error(tmp_sqlite_url):
    """Two sessions committing simultaneously must both succeed.

    Before the fix this reproduced 'database is locked' with an
    unprotected engine; with WAL + busy_timeout the second writer waits
